            ...
            <div key={index} className="p-6 bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 hover:bg-white/10 transition-all duration-300">
              ...
              <div className="flex flex-wrap gap-2">
                {category.skills.map((skill, idx) => (
                  <span 
                    key={idx} 
                    className="px-3 py-2 bg-gradient-to-r from-slate-700 to-slate-600 text-gray-200 text-sm rounded-lg border border-slate-500/30 hover:from-blue-600/20 hover:to-purple-600/20 hover:border-blue-500/30 transition-all duration-300 cursor-default"
                  >
                    {skill}
                  </span>
                ))}
